...
describe('エラーケース_ディナー付きプラン', () => {
  ...
  it('入力値が空白でエラーとなること', async () => {
    ...
    await PlansPage.openPlanByTitle('ディナー付きプラン');
  ...
  it('不正な入力値でエラーとなること_ディナー付きプラン_宿泊数（小）', async () => {
    await TopPage.open();
    await TopPage.goToLoginPage();
    await LoginPage.email.setValue('ichiro@example.com');
    await LoginPage.password.setValue('password');
  ...
  it('不正な入力値でエラーとなること_ディナー付きプラン_宿泊数（大）', async () => {
    await TopPage.open();
    await TopPage.goToLoginPage();
    await LoginPage.email.setValue('ichiro@example.com');
    await LoginPage.password.setValue('password');
    ...
    await ReservePage.reserveTerm.setValue('4');
    await ReservePage.headCount.setValue('5');
  ...
  it('不正な入力値でエラーとなること_ディナー付きプラン_確認_選択してください', async () => {
    await TopPage.open();
    await TopPage.goToLoginPage();
    await LoginPage.email.setValue('ichiro@example.com');
    await LoginPage.password.setValue('password');
